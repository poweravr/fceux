...
{
  int val;
  g_config->getOption("SDL.MergeControls", &val);
...
{
  int val;
  g_config->getOption("SDL.AnalogStick", &val);
  ...
  while (!done)
  {
    ...
    if( g_dirty )
    {
      ...
      for(i=0,y=72; i < CONTROL_MENUSIZE; i++,y+=15)
      {
        ...
        {
          int analogValue;
          g_config->getOption("SDL.AnalogStick", &analogValue);
          sprintf(cBtn, "%s", analogValue ? "on" : "off");
        }
