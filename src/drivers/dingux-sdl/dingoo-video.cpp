...
// STATIC GLOBALS
static int s_curbpp;
static int s_firstline, s_lastline;
static int s_tlines;
static int s_inited;
...
int InitVideo(FCEUGI *gi)
{
  ...
  FCEUI_GetCurrentVidSystem(&s_firstline, &s_lastline);
  s_tlines = s_lastline - s_firstline + 1;
...
void BlitScreen(uint8 *XBuf)
{
  ...
  register uint8* src = XBuf;
  if(s_fullscreen == 7)  // fullscreen 2xSaI 320x224, 320x240
  {
    switch(screen->w)
    {
      case 480:
        upscale_480x272((uint32 *)screen->pixels, (uint8 *)XBuf + 256 * 8);
        break;
      case 400:
        upscale_384x240((uint32 *)screen->pixels, (uint8 *)XBuf + 256 * 8);
        break;
      case 320:
  ...
  else if(s_fullscreen == 6)  // fullscreen new 320x224, 320x240
  {
    switch(screen->w)
    {
      case 480:
  ...
  else if(s_fullscreen == 5)  // aspect fullscreen 2xSaI 288x224, 288x240
  {
    switch(screen->w)
    {
      case 480:
        upscale_384x272((uint32 *)screen->pixels, (uint8 *)XBuf + 256 * 8);
        break;
      case 400:
      case 320:
        uint16* dest = (uint16*)screen->pixels;
  ...
  else if(s_fullscreen == 4)  // aspect fullscreen new 288x224, 288x240
  {
    switch(screen->w)
    {
      case 480:
