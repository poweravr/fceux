...
extern  uint16 s_psdl[256];
#define S_PSDL(s)       s_psdl[s]
#if 0
// OLD, some color mistake
#define BLIT_SRC4_DEST5(src, dst)   \
  {\
    register u32 c1, c2;\
    c1 = S_PSDL(src[0]);\
    c1 = (c1 | c1 << 16) & 0x07E0F81F;\
    c2 = ((c2 * 3 + c1 * 13) >> 4) & 0x07E0F81F;\
    dst[0] = c2 >> 16 | c2;\
    c2 = S_PSDL(src[1]);\
    c2 = (c2 | c2 << 16) & 0x07E0F81F;\
    c1 = ((c1 * 6 + c2 * 10) >> 4) & 0x07E0F81F;\
    dst[1] = c1 >> 16 | c1;\
    c1 = S_PSDL(src[2]);\
    c1 = (c1 | c1 << 16) & 0x07E0F81F;\
    c2 = ((c2 * 10 + c1 * 6) >> 4) & 0x07E0F81F;\
    dst[2] = c2 >> 16 | c2;\
    c2 = S_PSDL(src[3]);\
    dst[4] = c2;\
    c2 = (c2 | c2 << 16) & 0x07E0F81F;\
    c1 = ((c1 * 13 + c2 * 3) >> 4) & 0x07E0F81F;\
    dst[3] = c1 >> 16 | c1;\
  }
#define BLIT_SRC7_DEST8(src, dst)   \
  {\
    register u32 c1, c2;\
    c1 = S_PSDL(src[0]);\
    c1 = (c1 | c1 << 16) & 0x07E0F81F;\
    c2 = ((c2 + c1 * 7) >> 3) & 0x07E0F81F;\
    dst[0] = c2 >> 16 | c2;\
    c2 = S_PSDL(src[1]);\
    c2 = (c2 | c2 << 16) & 0x07E0F81F;\
...
#define BLIT_SRC8_DEST9(src, dst)   \
  {\
    register u32 c1, c2;\
    c1 = S_PSDL(src[0]);\
    dst[0] = c1;\
    ...
    c2 = S_PSDL(src[1]);\
    c2 = (c2 | c2 << 16) & 0x07E0F81F;\
    c1 = ((c1 + c2 * 7) >> 3) & 0x07E0F81F;\
    dst[1] = c1 >> 16 | c1;\
    /**/\
    c1 = S_PSDL(src[2]);\
    c1 = (c1 | c1 << 16) & 0x07E0F81F;\
    c2 = ((c2 + c1 * 3) >> 2) & 0x07E0F81F;\
    dst[2] = c2 >> 16 | c2;\
    /**/\
    c2 = S_PSDL(src[3]);\
    c2 = (c2 | c2 << 16) & 0x07E0F81F;\
    c1 = ((c1 * 3 + c2 * 5) >> 3) & 0x07E0F81F;\
    dst[3] = c1 >> 16 | c1;\
    /**/\
    c1 = S_PSDL(src[4]);\
    c1 = (c1 | c1 << 16) & 0x07E0F81F;\
    ...
    c2 = S_PSDL(src[5]);\
    c2 = (c2 | c2 << 16) & 0x07E0F81F;\
    c1 = ((c1 * 5 + c2 * 3) >> 3) & 0x07E0F81F;\
    dst[5] = c1 >> 16 | c1;\
    /**/\
    c1 = S_PSDL(src[6]);\
    c1 = (c1 | c1 << 16) & 0x07E0F81F;\
    c2 = ((c2 * 3 + c1) >> 2) & 0x07E0F81F;\
    dst[6] = c2 >> 16 | c2;\
    /**/\
    c2 = S_PSDL(src[7]);\
    dst[8] = c2;\
    c2 = (c2 | c2 << 16) & 0x07E0F81F;\
    c1 = ((c1 * 7 + c2) >> 3) & 0x07E0F81F;\
    dst[7] = c1 >> 16 | c1;\
  }

#endif

void upscale_256_to_288(uint16_t* dst, int dst_width, uint8* src, int src_height)
{
  dst_width -= 288;
  do
  {
...
void upscale_256_to_320(uint16_t* dst, int dst_width, uint8* src, int src_height)
{
  dst_width -= 320;
  do
  {
...
extern void Scale_2xSaI(u8* srcPtr, u32 srcPitch, u8* deltaPtr, u8* dstPtr, u32 dstPitch, u32 dstWidth, u32 dstHeight, int w, int h);
extern void palette_2xSaI(u8* srcPtr, u32 srcPitch, u8* deltaPtr, u8* dstPtr, u32 dstPitch, int width, int height, u16 palette[]);

#define RGB16_GRB32(c)      (((c) | (c) << 16) & 0x07E0F81F)
...
void upscale_256_to_320_filter(uint16_t* dst, int dst_width, uint8* src, int src_height)
{
  ...
  //
  {
    ...
    do
    {
      ...
      do
      {
        __builtin_prefetch(dst32 + 4, 1);
        *dst32++ = palettetranslate[src16[0]];
        *dst32++ = palettetranslate[src16[1]];
        *dst32++ = palettetranslate[src16[2]];
        *dst32++ = palettetranslate[src16[3]];
        src16 += 4;
      }
      while(--count);
    }
  ...
  //palette_2xSaI((u8*)src, 256, 0, (u8*)buffer2x, 512<<1, 256, src_height, s_psdl);
  half_2xSaI((u8*)dst, 256<<1, 0, (u8*)buffer2x, 512<<1, 256, src_height);
  //SuperEagle((u8*)dst, 256<<1, 0, (u8*)buffer2x, 512<<1, 256, src_height);
  //Super2xSaI((u8*)dst, 256<<1, 0, (u8*)buffer2x, 512<<1, 256, src_height);
  //Scale_2xSaI((u8*)buffer2x, 512<<1, 0, (u8*)dst, 320<<1, 320, 224, 512, src_height<<1);
  {
    register u16* src16 = (u16*)buffer2x;
    register u16* dst16 = (u16*)dst;
    do
    {
      int count = 64; //320 / 5;
      do
...
#define READ_SRC_TO(c, src)     c = src; c = (c << 16 | c) & 0x07E0F81F
#define BLIT_SRC16_DEST9(src, dst)   \
  {\
    register u32 c1, c2, c3;\
    READ_SRC_TO(c2, src[0]);\
    READ_SRC_TO(c1, src[1]);\
    c2 = ((c2 * 9 + c1 * 7) >> 4) & 0x07E0F81F;\
    dst[0] = c2 >> 16 | c2;\
    /**/\
    READ_SRC_TO(c2, src[2]);\
    READ_SRC_TO(c3, src[3]);\
    c1 = ((c1 * 2 + c2 * 9 + c3 * 5) >> 4) & 0x07E0F81F;\
    dst[1] = c1 >> 16 | c1;\
    /**/\
    READ_SRC_TO(c2, src[4]);\
    READ_SRC_TO(c1, src[5]);\
    c3 = ((c3 * 4 + c2 * 9 + c1 * 3) >> 4) & 0x07E0F81F;\
    dst[2] = c3 >> 16 | c3;\
    /**/\
    READ_SRC_TO(c2, src[6]);\
    READ_SRC_TO(c3, src[7]);\
    c1 = ((c1 * 6 + c2 * 9 + c3) >> 4) & 0x07E0F81F;\
    dst[3] = c1 >> 16 | c1;\
    /**/\
    READ_SRC_TO(c1, src[8]);\
    c3 = ((c3 + c1) >> 1) & 0x07E0F81F;\
    dst[4] = c3 >> 16 | c3;\
    /**/\
    READ_SRC_TO(c2, src[9]);\
    READ_SRC_TO(c3, src[10]);\
    c1 = ((c1 + c2 * 9 + c3 * 6) >> 4) & 0x07E0F81F;\
    dst[5] = c1 >> 16 | c1;\
    /**/\
    READ_SRC_TO(c2, src[11]);\
    READ_SRC_TO(c1, src[12]);\
    c3 = ((c3 * 3 + c2 * 9 + c1 * 4) >> 4) & 0x07E0F81F;\
    dst[6] = c3 >> 16 | c3;\
    /**/\
    READ_SRC_TO(c2, src[13]);\
    READ_SRC_TO(c3, src[14]);\
    c1 = ((c1 * 5 + c2 * 9 + c3 * 2) >> 4) & 0x07E0F81F;\
    dst[7] = c1 >> 16 | c1;\
    /**/\
    READ_SRC_TO(c1, src[15]);\
    c3 = ((c3 * 7 + c1 * 9) >> 4) & 0x07E0F81F;\
    dst[8] = c3 >> 16 | c3;\
  }
...
void upscale_256_to_288_filter(uint16_t* dst, int dst_width, uint8* src, int src_height)
{
  ...
  //
  {
    ...
    do
    {
      ...
      while(--count);
      dst32 += 32;//(320 - 256) * sizeof(u16) / sizeof(u32);
    }
    while(--height);
  }

  //palette_2xSaI((u8*)src, 256, 0, (u8*)buffer2x, 512<<1, 256, src_height, s_psdl);
  half_2xSaI((u8*)dst, 320<<1, 0, (u8*)buffer2x, 512<<1, 256, src_height);
  //SuperEagle((u8*)dst, 256<<1, 0, (u8*)buffer2x, 512<<1, 256, src_height);
  //Super2xSaI((u8*)dst, 256<<1, 0, (u8*)buffer2x, 512<<1, 256, src_height);
  //Scale_2xSaI((u8*)buffer2x, 512<<1, 0, (u8*)dst, 320<<1, 320, 224, 512, src_height<<1);
  dst_width -= 288;
