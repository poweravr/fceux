...
static int InFrame;
double fps_scale = Normal; // used by dingoo.cpp
...
void IncreaseEmulationSpeed(void)
{
  ...
  if(fps_scale > Fastest) fps_scale = Fastest;

  RefreshThrottleFPS();

  printf("emulation speed %.1f%%", fps_scale*100.0);
}
...
void DecreaseEmulationSpeed(void)
{
  ...
  fps_scale /= LOGMUL;
  if(fps_scale < Slowest)
    fps_scale = Slowest;

  RefreshThrottleFPS();

  FCEU_DispMessage("emulation speed %.1f%%", fps_scale*100.0);
}
