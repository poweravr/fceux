  ...
  for (x = 0; x < MMC5_NOCARTS; x++)
  {
    if (crc32 == MMC5CartList[x].crc32)
    {
      if(MMC5CartList[x].size > 1)
        printf(" >8KB external WRAM present.  Use UNIF if you hack the ROM image.\n");
...
static void BuildWRAMSizeTable(void)
{
  ...
  for (x = 0; x < 8; x++)
  {
    switch (MMC5WRAMsize)
    {
      ...
      case 1:
        MMC5WRAMIndex[x] = (x > 3) ? 255 : 0;
        break;        //0,0,0,0,X,X,X,X
      case 2:
        MMC5WRAMIndex[x] = (x & 4) >> 2;
