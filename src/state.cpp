  ...
  while(totalsize > 0)
  {
    ...
    switch(t)
    {
      case 1:
        ...
        break;
      case 3:
        if(!ReadStateChunk(is,FCEUPPU_STATEINFO,size)) ret=false;
        break;
      case 31:
        if(!ReadStateChunk(is,FCEU_NEWPPU_STATEINFO,size)) ret=false;
        break;
      case 4:
        if(!ReadStateChunk(is,FCEUCTRL_STATEINFO,size)) ret=false;
        break;
      case 7:
...
bool FCEUSS_SaveMS(EMUFILE* outstream, int compressionLevel)
{
  ...
  if(len != totalsize)
  {
    printf("sanity violation: len != totalsize");
    return false;
  }
...
void FCEUSS_Save(const char *fname, bool display_message)
{
  ...
  {
    //printf("daCurrentState=%d",CurrentState);
...
void AddExState(void *v, uint32 s, int type, char *desc)
{
  ...
  else
  {
    ...
    {
      once=0;
      printf("Error in AddExState: SFEXINDEX overflow.\nSomebody made SFMDATA_SIZE too small.");
    }
  }
...
int FCEUI_SelectState(int w, int show)
{
  ...
  if(w == -1)
  {
    StateShow = 0;  //mbg merge 7/17/06 had to make return a value
    return 0;
  }
...
void SwapSaveState()
{
  ...
  if (!lastSavestateMade)
  {
    ...
    printf("Undo savestate was attempted but unsuccessful because there was not a recently used savestate.\n");
  ...
  {
    FCEUI_DispMessage("Can't Undo",0);
    printf("Undo savestate was attempted but unsuccessful because there was not a backup of the last used savestate.\n");
