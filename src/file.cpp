  ...
  while(fread(header,1,3,ips)==3)
  {
    ...
    if(!size)	/* RLE */
    {
      ...
      if((offset+size)>(uint32)fp->size)
      {
        ...
        if(!buf)
        {
          printf("  Oops.  IPS patch %d(type RLE) goes beyond end of file.  Could not allocate memory.\n",count);
          goto end;
        }
    ...
    else		/* Normal patch */
    {
      ...
      if((offset+size)>(uint32)fp->size)
      {
        ...
        if(!buf)
        {
          printf("  Oops.  IPS patch %d(type normal) goes beyond end of file.  Could not allocate memory.\n",count);
          goto end;
        }
        memset(buf+fp->size,0,offset+size-fp->size);
      }
...
inline FileBaseInfo DetermineFileBase(const std::string& str)
{
...
FCEUFILE * FCEU_fopen(const char *path, const char *ipsfn, char *mode, char *ext, int index, const char** extensions)
{
  ...
  {
    printf("invalid file open mode specified (only wb and rb are supported)");
    return 0;
  }
...
{
  //	printf("odirs[%d]=%s->%s", which, odirs[which], n);
...
std::string FCEU_MakeFName(int type, int id1, const char *cd1)
{
  ...
  switch(type)
  {
    ...
    case FCEUMKF_IPS:
      ...
      break;
    case FCEUMKF_GGROM:
      sprintf(ret,"%s" PSS "gg.rom",BaseDirectory.c_str());
      break;
    case FCEUMKF_FDSROM:
    ...
    case FCEUMKF_MOVIEGLOB2:
      sprintf(ret,"%s" PSS "*.???",BaseDirectory.c_str());
      break;
