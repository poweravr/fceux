...
static int DoMirroring(FCEUFILE *fp)
{
  ...
  {
    printf(" Incorrect Mirroring Chunk Size (%d). Data is:", uchead.info);
...
static int CTRL(FCEUFILE *fp)
{
  ...
  {
    printf(" Incorrect Control Chunk Size (%d). Data is:", uchead.info);
    for (i = 0; i < uchead.info; i++)
    {
      t = FCEU_fgetc(fp);
      printf(" %02x", t);
