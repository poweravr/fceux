...
void MovieRecord::parse(MovieData* md, EMUFILE* is)
{
  ...
  if(md->fourscore)
  {
    parseJoy(is,joysticks[0]);
    is->fgetc(); //eat the pipe
    parseJoy(is,joysticks[1]);
    is->fgetc(); //eat the pipe
    parseJoy(is,joysticks[2]);
    is->fgetc(); //eat the pipe
    parseJoy(is,joysticks[3]);
    is->fgetc(); //eat the pipe
  }
  else
  {
...
void MovieRecord::dump(MovieData* md, EMUFILE* os, int index)
{
  ...
  {
    os->fputc('|');
    dumpJoy(os,joysticks[0]);
    os->fputc('|');
    dumpJoy(os,joysticks[1]);
    os->fputc('|');
    dumpJoy(os,joysticks[2]);
    os->fputc('|');
    dumpJoy(os,joysticks[3]);
    os->fputc('|');
  }
  else
  {
    for(int port=0; port<2; port++)
    {
      ...
      {
        putdec<uint8,3,true>(os,zappers[port].x);
        os->fputc(' ');
        putdec<uint8,3,true>(os,zappers[port].y);
        os->fputc(' ');
        putdec<uint8,1,true>(os,zappers[port].b);
        os->fputc(' ');
        putdec<uint8,1,true>(os,zappers[port].bogo);
        os->fputc(' ');
...
bool FCEUI_LoadMovie(const char *fname, bool _read_only, int _pauseframe)
{
  ...
  {
    printf("Cannot open a movie in read+write from an archive.");
...
static void openRecordingMovie(const char* fname)
{
  ...
  if(!osRecordingMovie)
    printf("Error opening movie output file: %s",fname);
  strcpy(curMovieFilename, fname);
#if defined(WIN32) && !defined(DINGUX)
...
void FCEUMOV_AddCommand(int cmd)
{
  ...
  switch (cmd)
  {
    ...
    case FCEUNPCMD_FDSINSERT:
      cmd = MOVIECMD_FDS_INSERT;
      break;
    case FCEUNPCMD_FDSSELECT:
      cmd = MOVIECMD_FDS_SELECT;
      break;
...
bool FCEUMOV_ReadState(EMUFILE* is, uint32 size)
{
  ...
  if (!movie_readonly)
  {
...
#else
      printf("This movie is a TAS Editor project file! It can be modified in TAS Editor only.\nMovie is now Read-Only.\n");
#endif
      movie_readonly = true;
    }
    if (FCEU_isFileInArchive(curMovieFilename))
    {
      ...
      printf("Cannot loadstate in Read+Write with movie from archive. Movie is now Read-Only.");
      movie_readonly = true;
    }
  }
  ...
  if(!LoadFM2(tempMovieData, is, size, false))
  {
    ...
    if(FCEU_state_loading_old_format)
    {
      if(movieMode == MOVIEMODE_PLAY || movieMode == MOVIEMODE_RECORD || movieMode == MOVIEMODE_FINISHED)
      {
        ...
        printf("You have tried to use an old savestate while playing a movie. This is unsupported (since the old savestate has old-format movie data in it which can't be converted on the fly)");
      }
    }
    return false;
  }
  ...
  if(movieMode == MOVIEMODE_PLAY || movieMode == MOVIEMODE_RECORD || movieMode == MOVIEMODE_FINISHED)
  {
    ...
    if(tempMovieData.guid != currMovieData.guid)
    {
      ...
      if(result == IDCANCEL)
      {
        ...
        {
          printf("Unable to restore backup, movie playback stopped.");
          FCEUI_StopMovie();
        }
    ...
    if (movie_readonly)
    {
      ...
      else if (currFrameCounter > (int)currMovieData.records.size())
      {
        ...
        if (!backupSavestates)	//If backups are disabled we can just resume normally since we can't restore so stop movie and inform user
        {
          printf("Error: Savestate is from a frame (%d) after the final frame in the movie (%d). This is not permitted.\nUnable to restore backup, movie playback stopped.", currFrameCounter, currMovieData.records.size()-1);
          FCEUI_StopMovie();
        }
        else
          printf("Savestate is from a frame (%d) after the final frame in the movie (%d). This is not permitted.", currFrameCounter, currMovieData.records.size()-1);
        return false;
      }
      else if (currFrameCounter > (int)tempMovieData.records.size())
      {
        ...
        if (!backupSavestates)	//If backups are disabled we can just resume normally since we can't restore so stop movie and inform user
        {
          printf("Error: Savestate is from a frame (%d) after the final frame in the savestated movie (%d). This is not permitted.\nUnable to restore backup, movie playback stopped.", currFrameCounter, tempMovieData.records.size()-1);
          FCEUI_StopMovie();
        }
        else
          printf("Savestate is from a frame (%d) after the final frame in the savestated movie (%d). This is not permitted.", currFrameCounter, tempMovieData.records.size()-1);
        return false;
      }
      else
      {
