...
void CheckBad(uint64 md5partial)
{
  ...
  while (BadROMImages[x].name)
  {
    ...
    {
      printf("The copy game you have loaded, \"%s\", is bad, and will not work properly in FCEUX.", BadROMImages[x].name);
      return;
    }
    x++;
  }
...
int iNESLoad(const char *name, FCEUFILE *fp, int OverwriteVidMode)
{
  ...
  printf(" PRG ROM:  %3d x 16KiB\n", (round) ? ROM_size: not_round_size);
  printf(" CHR ROM:  %3d x  8KiB\n", head.VROM_size);
  printf(" ROM CRC32:  0x%08lx\n", iNESGameCRC32);
  {
    int x;
    printf(" ROM MD5:  0x");
    for(x=0; x<16; x++)
      printf("%02x",iNESCart.MD5[x]);
    printf("\n");
  }
  ...
  */
  ...
  if (!iNES_Init(MapperNo))
    printf("iNES mapper #%d is not supported at all.", MapperNo);
