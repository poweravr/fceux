...
static DECLFR(FDSSRead)
{
  ...
  {
    case 0x0:
      return(amplitude[0] | (X.DB & 0xC0));
    case 0x2:
      return(amplitude[1] | (X.DB & 0xC0));
  }
  return(X.DB);
}
...
int FDSLoad(const char *name, FCEUFILE *fp)
{
  ...
  {
    printf("FDS BIOS ROM image missing: %s", FCEU_MakeFName(FCEUMKF_FDSROM, 0, 0).c_str());
    free(fn);
    return 0;
  }
  ...
  {
    fclose(zp);
    printf("FDS BIOS ROM image incompatible: %s", FCEU_MakeFName(FCEUMKF_FDSROM, 0, 0).c_str());
    return 0;
  }
  ...
  if (fread(FDSBIOS, 1, FDSBIOSsize, zp) != FDSBIOSsize)
  {
    ...
    FDSBIOS = NULL;
    fclose(zp);
    printf("Error reading FDS BIOS ROM image.");
    return 0;
  }
  ...
  if (!disableBatteryLoading)
  {
    ...
    {
      printf("Disk was written. Auxillary FDS file open \"%s\".\n",fn);
      FreeFDSMemory();
      if (!SubLoad(tp))
      {
        printf("Error reading auxillary FDS file.");
        if(FDSBIOS)
          free(FDSBIOS);
        FDSBIOS = NULL;
        free(fn);
